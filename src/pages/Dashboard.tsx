    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-2)' }}>Your AI workforce at a glance</p>
        </div>
          ...
          <div key={a.id} onClick={() => nav(`/chat/${a.id}`)} className="rounded-2xl p-4 bg-white cursor-pointer group hover:shadow-lg hover:shadow-black/5 transition-all" style={{ border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate" style={{ color: 'var(--text)' }}>{a.name}</p>
                <p className="text-[11px] truncate" style={{ color: a.color }}>{a.role}</p>
              </div>
              <div className="w-2 h-2 rounded-full" style={{ background: a.status === 'active' ? '#00d4aa' : a.status === 'working' ? '#FFA726' : '#FF6B6B' }} />
            </div>
            <div className="flex gap-1 flex-wrap">
              ...
              {a.capabilities.length > 2 && <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'var(--bg-2)', color: 'var(--text-3)' }}>+{a.capabilities.length - 2}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card>
          ...
          {tasks.length === 0 ? (
            <p className="text-xs py-6 text-center" style={{ color: 'var(--text-3)' }}>No tasks yet. Talk to Atlas to get started.</p>
          ) : (
              ...
              {tasks.slice(-5).reverse().map(t => {
                ...
                return (
                  <div key={t.id} className="flex items-center gap-2 py-1.5">
                    ...
                    <span className="text-xs flex-1 truncate" style={{ color: 'var(--text-2)' }}>{t.title}</span>
                    {t.status === 'completed' && <CheckCircle size={12} style={{ color: '#00d4aa' }} />}
                    {t.status === 'in-progress' && <Loader2 size={12} className="animate-spin" style={{ color: '#FFA726' }} />}
                    {t.status === 'pending' && <Clock size={12} style={{ color: 'var(--text-3)' }} />}
                  </div>
                );
              })}
            </div>
        ...
        <Card>
          ...
          {logs.length === 0 ? (
            <p className="text-xs py-6 text-center" style={{ color: 'var(--text-3)' }}>Agent activity will appear here.</p>
          ) : (
            <div className="space-y-2">
              {logs.slice(-5).reverse().map(l => {
                ...
                return (
                  <div key={l.id} className="flex items-center gap-2 py-1.5">
                    <span className="text-sm">{agent?.avatar}</span>
                    <span className="text-xs flex-1 truncate" style={{ color: 'var(--text-2)' }}><strong style={{ color: agent?.color }}>{agent?.name}</strong> {l.action}</span>
                    <span className="text-[10px]" style={{ color: 'var(--text-3)' }}>{new Date(l.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                );
              })}
            </div>
